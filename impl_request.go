...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/Remedios14/go-lark-project/internal"
	"io"
	"io/ioutil"
	"mime"
	"mime/multipart"
...
func (r *LarkProject) RawRequest(ctx context.Context, req *RawRequestReq, resp interface{}) (response *Response, err error) {
...
type RawRequestReq struct {
	...
	NeedPluginToken bool        // need plugin token
	NeedUserKey     bool        // need user key
	//MethodOption          *MethodOption // method option, such as userAccessToken
}

// 把可读的 RawRequestReq ，解析为 http 请求的参数 rawHttpRequestParam
func (r *LarkProject) parseRawHttpRequest(ctx context.Context, req *RawRequestReq) (*rawHttpRequest, error) {
	// 0 init
...
func (r *LarkProject) doRequest(ctx context.Context, rawHttpReq *rawHttpRequest, realResponse interface{}) (*Response, error) {
...
func (r *rawHttpRequest) parseHeader(ctx context.Context, ins *LarkProject, req *RawRequestReq) error {
	...
	if req.NeedPluginToken {
		token, _, err := ins.Auth.GetPluginToken(ctx)
		if err != nil {
			return err
		}
		r.Headers[HttpHeaderPluginToken] = token.Token
	}
	if req.NeedUserKey {
		r.Headers[HttpHeaderUserKey] = req.UserKey
	}
